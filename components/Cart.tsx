...
//add product to shoppingCart
  ...
  return (
    <div className="w-[300px] md:w-[360px] lg:w-[360px] h-[95vh]  lg:h-[100vh] flex-col fixed right-0 border border-black rounded-lg bg-white  ">
      ...
      {items.length ? (
        <>
          <div className="w-full h-[55%] lg:h-[60%] px-4 overflow-y-auto">
            {items.map((item) => (
...
export function ShoppingCartModal() {
  return (
    <Dialog.Root>
      ...
      <Dialog.Content className="data-[state=open]:animate-displayCart fixed top-0 right-0 w-auto translate-x-[-50%] translate-y-[-50%] rounded-[6px] z-20">
        <ShoppingCart />
      </Dialog.Content>
    </Dialog.Root>
  );
